...
export class TblPurchaseOrderDetailUpdateComponent implements OnDestroy {
  ...
  savedPurchaseOrderDetails: TblPurchaseOrderDetail[] = [];
  ...
  ngOnInit(): void {
    ...
    this.fldFKPo = poIdParam ? parseInt(poIdParam, 10) : null;
    ...
    this.detailLoadSubscription = this.tblPurchaseOrderDetailService
      ...
      .subscribe({
        next: (response) => {
          ...
          this.fldFKPo = this.fldFKPo || Number(this.model.fldFKPo) || null;
          if (!this.poHeaderInfo.poNo) {
            this.loadPurchaseOrderHeader();
          }
  ...
  private loadPurchaseOrderHeader(): void {
    ...
    this.headerLoadSubscription = this.tblPurchaseOrderService
      ...
      .subscribe({
        ...
        error: (err) => {
          this.toastr.error(
            err?.error?.message || "Unable to load Purchase Order information.",
            "PO Load Failed",
            { toastClass: "ngx-toastr custom-toast error-toast" },
          );
        },
  ...
  OnFormSubmit(form: NgForm, requestedAction: "Edit" | "Delete"): void {
    ...
    if (!this.model.fldFKPo || this.model.fldFKPo <= 0) {
      this.recoverAfterSubmitFailure(
        "Purchase Order information is unavailable. Please return to the PO list and open Details again.",
      );
      return;
    }
  ...
  private captureSavedDetails(response: unknown): void {
    ...
    const rowsToDisplay = validRows.length
      ? validRows
      : [{ ...this.model } as unknown as TblPurchaseOrderDetail];
  ...
  private navigateBackToDetails(): void {
    if (!this.fldFKPo || this.fldFKPo <= 0) {
      this.navigateBackToPurchaseOrders();
      return;
    }

    this.router
      .navigate(["/transactiontables/tblPurchaseOrderDetail", this.fldFKPo], {
        queryParams: {
  ...
  private navigateBackToPurchaseOrders(): void {
    this.router.navigate(["/transactiontables/tblPurchaseOrder"], {
      queryParams: {
        fromDate: this.poHeaderInfo.fromDate,
        toDate: this.poHeaderInfo.toDate,
      },
